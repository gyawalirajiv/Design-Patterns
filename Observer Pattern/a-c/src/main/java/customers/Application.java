...
public class Application {
	...
	public static void main(String[] args) {
		...
		Iterator<Customer> addressFilterIteratorByCity = customerCollection
				.getAddressFilterIterator(c -> c.getAddress().getCity().equals("Chicago"));
		while (addressFilterIteratorByCity.hasNext()){
			System.out.println(addressFilterIteratorByCity.next());
		}
		System.out.println("========");

		Iterator<Customer> addressFilterIteratorByZip = customerCollection
				.getAddressFilterIterator(c -> c.getAddress().getZip().startsWith("12"));
		while (addressFilterIteratorByZip.hasNext()){
			System.out.println(addressFilterIteratorByZip.next());
		}

		System.out.println("==========");

		Iterator<Customer> skipOneIterator = customerCollection.getSkipOneIterator();
		while (skipOneIterator.hasNext()){
			System.out.println(skipOneIterator.next());
		}
	}
}
